...
class hand_env_base:
    def __init__(self):
        ...
        self.target_inputs, _ = self.generate_inputs(np.array([0.42, 0.22, 0.80, 0.850], dtype=np.float32))
    ...
    def take_action(self, input_state, action, delta=0.2):
        ...
        if np.max(new_input_state) > 1.0 or np.min(new_input_state) < -1.0:
