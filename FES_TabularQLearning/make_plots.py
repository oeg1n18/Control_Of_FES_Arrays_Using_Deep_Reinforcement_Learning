...
target_ang_sum = np.sum(2.0944 + env.target_angles[0, :, 0]) + np.sum(1.5708 + env.target_angles[0, :, 1]) + np.sum(1.0472 + env.target_angles[0, :, 2])
error = np.sum(np.abs(env.target_angles[0, :, 0] - outputs[:, 0])) + np.sum(np.abs(env.target_angles[0, :, 1] - outputs[:, 1]))  + np.sum(np.abs(env.target_angles[0, :, 2] - outputs[:, 2]))
print("Percentage_error = ", (error/target_ang_sum) *100)

ax.set_xlabel("Time (s)")
ax.set_ylabel("Angle (rad)")
ax.set_title("Tabular Double Q Learning")
plt.legend()
print("hello")
plt.show()
